...
func run(t) -> void:
	...
	t.is_true(base_source.contains("MATERIAL_BRIGHTNESS_BOOST := 0.22"), "所有地图材质应统一进一步提亮")
	t.is_true(base_source.contains("func _improve_material_color"), "地图材质应通过公共入口改善颜色")
	t.is_true(base_source.contains("MATERIAL_TEXTURE_CONTRAST := 1.45"), "地图纹理应进一步增强细节对比")
	t.is_true(base_source.contains("MATERIAL_TEXTURE_SIZE := 64"), "地图纹理应提升分辨率")
	...
	t.is_true(model_source.contains("mat.metallic = 0.80"), "Gun metal should have a stronger metallic finish")
	t.is_true(model_source.contains("mat.emission_energy_multiplier = 1.95"), "Highlighted parts should have premium glow")
