...
func run(t) -> void:
	...
	t.is_true(base_source.contains("MATERIAL_BRIGHTNESS_BOOST := 0.25"), "所有地图材质应统一进一步提亮")
	t.is_true(base_source.contains("func _improve_material_color"), "地图材质应通过公共入口改善颜色")
	t.is_true(base_source.contains("MATERIAL_TEXTURE_CONTRAST := 1.62"), "地图纹理应进一步增强细节对比")
	t.is_true(base_source.contains("MATERIAL_TEXTURE_SIZE := 96"), "地图纹理应提升分辨率")
	t.is_true(base_source.contains("sun.shadow_enabled = true"), "顶配画质应启用地图阴影")
	...
	t.is_true(model_source.contains("func _make_premium_texture"), "Characters and weapons should use sharper premium textures")
	...
	t.is_true(model_source.contains("Image.create(64, 64"), "Characters and weapons should use higher resolution premium textures")
	t.is_true(model_source.contains("mat.metallic = 0.88"), "Gun metal should have a stronger metallic finish")
	t.is_true(model_source.contains("mat.emission_energy_multiplier = 2.25"), "Highlighted parts should have premium glow")
