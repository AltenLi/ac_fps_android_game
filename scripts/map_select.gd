...
func _build_ui() -> void:
	...
	enter.pressed.connect(func() -> void:
		GameSettings.set_selected_map(MapRegistry.MAPS[_selected_index]["id"])
...
func _make_card(index: int) -> Control:
	...
	var is_locked := not PlayerData.has_map(data["id"])
...
func _show_purchase_dialog(index: int) -> void:
	var data: Dictionary = MapRegistry.MAPS[index]
	## 全屏半透明遮罩
	...
	var desc := Label.new()
	desc.text = "%s\n消耗 10⭐（当前 %d⭐）" % [data["name"], PlayerData.total_stars]
	...
	hint.add_theme_color_override("font_color", Color(1.0, 0.32, 0.28, 1))
	...
	var confirm_btn := _make_button("确认购买", true)
	confirm_btn.custom_minimum_size = Vector2(130, 48)
	confirm_btn.pressed.connect(func() -> void:
		if PlayerData.purchase_map(data["id"]):
			blocker.queue_free()
			## 购买成功：重建界面，自动选中该地图
			_selected_index = index
			for child in get_children():
