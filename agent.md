...
最终目标是制作一款在手机上操控很容易、视觉精致好看的 3D 第一人称 5v5 枪战游戏。整体设计以大型战术 FPS 的品质感为目标参考，例如《无畏契约》这类移动端/大型 FPS 的清晰读图、精致场景、顺手触控、强反馈和低学习成本；但只参考品质标准和交互体验，不复制其地图、角色、商标、美术资产或具体受版权保护内容。项目不再以低模为视觉目标，而是通过更多多边形、程序化纹理、灯光、真实街景道具和移动端易用操作逐步推进到可发布、可测试、可商业化接入的预发布版本。玩家加入蓝队，与 4 名队友 AI 对战 5 名橙队 AI；单局 5 分钟，到时间按剩余人数判胜，一方全灭可提前结算。
...
- 地图选择：11 张原创程序化真实风格地图，前 3 张免费，其余使用星星解锁。
- 武器：RPG、M416、巴雷特。
...
- 2026-05-05：实现首页、设置页、地图选择页、早期基础城市地图、玩家 FPS 控制、RPG/M416/巴雷特、5v5 AI、5 分钟计时、胜负结算、HUD、触屏控件和导出说明。
...
- 2026-05-10：启动高级真实地图实验，先重做 `night_city` 霓虹都市：`BaseMap` 增加柏油、混凝土、喷漆、玻璃、全息屏程序化贴图材质；夜城加入湿润黑色道路、人行道、斑马线、水洼反光、多层玻璃高楼、立面发光肋条、全息广告扫描线、通风口、井盖、服务线缆、路桩和少量局部霓虹灯，并补充测试保护这些真实街区细节。
- 2026-05-10：修复移动端观战体验：`mobile_controls.gd` 增加 `_action_button_layer`，玩家死亡观战或比赛结算时根据 `can_accept_mobile_input()` 隐藏开火、左侧开火、跳跃、装弹、切枪按钮，并在按钮回调中加入战斗状态保护，避免观战时显示无法点击的操作按钮。
- 2026-05-10：确认项目视觉目标从“低模”调整为偏真实 3D：同步更新 `agent.md`、`README.md`、`PLAN.md`、`design/gdd/game-concept.md`、`docs/store-listing.md` 和首页文案；`BaseMap` 将圆柱/球体细分提高，并新增 `_create_trash_bin()`、`_create_lamp_post()` 真实街景道具，城市/工厂/海港/夜城主题加入灯柱、垃圾桶，`night_city` 额外加入 `StreetTrashBin`、`NightLampPost` 等高细节道具。
- 2026-05-10：明确最终产品方向：做成手机上操控很容易的精致 3D FPS；以《无畏契约》这类大型战术 FPS 的清晰读图、顺手触控、强反馈和精致场景作为品质参考，但只参考体验标准，不复制地图、角色、商标、美术资产或具体受版权保护内容。后续需求优先级：移动端易操控 > 清晰敌我/地图动线 > 精致真实场景 > 强战斗反馈 > 商业化接入。
- 2026-05-10：为整个游戏增加原创程序化音乐：`SoundManager` 新增 `play_menu_music()` 与 `play_combat_music()`，首页/地图选择/设置页播放冷静电子菜单音乐，战斗开始播放更紧张的战斗音乐；Android 默认不使用 WAV 原生循环，改用单段结束后重启，保留音频稳定性。
- 2026-05-10：优化地图解锁星星不足体验：`map_select.gd` 的“获取星星”改为“补足星星”，奖励数量不再固定 +3，而是通过 `_reward_amount_for_unlock(cost)` 按当前地图缺口 `maxi(1, cost - PlayerData.total_stars)` 一次性补足，调试/广告奖励入口可重复触发。
- 2026-05-10：基于 Claude-Code-Game-Studios 重新梳理 `PLAN.md`：当前定位为 Pre-Production / Polish 前置阶段，后续按手机易操控、清晰战术读图、精致真实观感、强即时反馈、移动端稳定性能、可商业化闭环六大支柱推进；路线分为基线指标、移动端易操控、战斗反馈和枪感、夜城样板地图、AI 战术性、全地图精致化、留存商业化、发布候选八个 sprint。
- 2026-05-10：整体优化 AI 寻路防卡墙：`AIController` 借鉴 Context Steering / Detour 思路，把原来简单左右二选一避障升级为扇形多方向采样、清障距离打分、避障方向短时锁定、目标进展检测；AI 如果虽然在移动但没有靠近目标，会进入脱困方向并必要时推进航点，减少“左走两步、右走两步”的墙边抖动。
- 2026-05-10：继续升级 AI 为“全局路径 + 局部避障”两层导航：`MatchManager` 基于每张地图的巡逻点、双方出生点、弹药点生成 `AStar3D` 地图级导航图，过滤被墙体阻挡的连接；`AIController` 缓存并定期刷新 `get_navigation_path()` 返回的路径，先走全局中间点，再使用 context steering 做近距离避障，作为没有完整 NavMesh 前的稳定寻路方案。
- 2026-05-10：整理面向新微信小游戏项目的完整设计文档 `docs/wechat-minigame-design-brief.md`，覆盖产品定位、核心循环、比赛规则、武器、触控、AI、地图、星星奖励、UI/HUD、音频、微信平台能力、MVP 裁剪、性能预算和验收清单；明确实时 3D FPS 应优先做微信小游戏而不是普通小程序。
